...
## Test Framework
Vitest via `@angular/build:unit-test`. All test files use `*.spec.ts`.

## Commands
- Run all unit tests: `npm test`. Bundles all 27+ spec files into one process — on low-memory systems (≈6GB), the build + JIT compilation may OOM. If killed, retry once memory frees up.
- Run one spec: `npm run test:file -- src/app/path/to/file.spec.ts` (**not** `npm test -- <file>` — that fails).
- E2E (auto starts dev server): `npm run e2e`.
- Direct Playwright run (start app first): `npx playwright test e2e/termo.spec.ts` (no `webServer` in `playwright.config.ts`).
